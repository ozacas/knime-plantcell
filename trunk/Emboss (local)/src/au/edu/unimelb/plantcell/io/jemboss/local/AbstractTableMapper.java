...
import au.edu.unimelb.plantcell.io.jemboss.settings.DummySetting;
...
public abstract class AbstractTableMapper {
	...
	public void setFormattedCells(Map<String, DataCell> created_files_map) {
		if (created_files_map != null) {
			for (DataColumnSpec s : m_f_col2ps.keySet()) {
				if (created_files_map.containsKey(s.getName())) {
					int idx = m_formatted_cols.indexOf(s);
					m_formatted_cells[idx] = created_files_map.get(s.getName());
				} else {
					int idx = m_formatted_cols.indexOf(s);
