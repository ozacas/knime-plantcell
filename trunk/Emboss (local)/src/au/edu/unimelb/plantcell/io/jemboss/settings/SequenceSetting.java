...
public class SequenceSetting extends StringSetting {
	...
	protected void setSourceFromColumn(boolean is_from_col) {
		m_from_column = is_from_col;
	}
	 ...
	 * @return
	 */
	protected JPanel make_sequence_panel(DataTableSpec dt) {
		...
		if (! isInputFromColumn()) {
			String val = getValue();
			File f = new File(val);
			if (val.length() > 0 && (!f.exists() || !f.canRead())) {
	...
	public void marshal(String id, DataCell c, PrintWriter fw) 
		...
		if (c.getType().isCompatible(StringValue.class)) {
			StringValue sv = (StringValue) c;
			char[]    seq = sv.getStringValue().toCharArray();
			...
			throw new InvalidSettingsException("Invalid cell for ID: "+id);
		}
	}
	...
	public void getArguments(ProgramSettingsListener l) throws InvalidSettingsException,IOException {
	    ...
	    } else if (t.equals("outseq") || t.equals("seqoutall") || t.equals("seqoutseq") || t.equals("seqout")) {
